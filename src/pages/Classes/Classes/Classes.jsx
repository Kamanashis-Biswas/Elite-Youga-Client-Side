...
const Classes = () => {
  ...
  useEffect(() => {
    fetch("https://eliteyouga-server-side.vercel.app/all-class")
      .then((data) => data.json())
      .then((data) => {
  ...
  const addClass = async ({ classId }) => {
    const resp = await fetch(
      "https://eliteyouga-server-side.vercel.app/set-class",
      {
        method: "POST",
  ...
  return (
    <>
      ...
      <div className="md:grid grid-cols-3 p-3 md:p-0">
        {cls.map((c) => (
